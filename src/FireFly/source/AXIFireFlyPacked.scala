...
case class AXIFireFlyPacked(
  ...
  afterInit clearWhen conv.io.done

  val apdone = Bool() setAsReg() init false
  apdone setWhen conv.io.done && afterInit
  ...
  val mm2sCmdGen = utils.FixedLenStridedAddrCmdGen(
    widthOfBnd = List(numOfIFMs.getWidth, numOfTimeSteps.getWidth, numOfOFMs.getWidth),
    widthOfInc = List(24, 24, 0),
    addrWidth = 24, lenWidth = 23
  )
  mm2sCmdGen.io.bound(0) := numOfIFMs
  mm2sCmdGen.io.bound(1) := numOfTimeSteps
  mm2sCmdGen.io.bound(2) := numOfOFMs
  mm2sCmdGen.io.inc(0) := strideOfChannel
  mm2sCmdGen.io.inc(1) := strideOfTimeStep
  mm2sCmdGen.io.inc(2) := U(0)
  ...
  configCtrl.read(mm2sCmdGen.io.busy, 0x50, 0)
  configCtrl.read(s2mmCmdGen.io.busy, 0x50, 32)
  ...
  io.mm2sCmd << utils.GenAxiDataMoverCmd(mm2sCmdGen.io.cmd, mm2sBaseAddr)
