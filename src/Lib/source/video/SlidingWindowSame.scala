...
object SlidingWindowSame {
  def apply[T <: Data](dataType: HardType[T], kernelSize: Int, maxWidth: Int) = new SlidingWindowSame(dataType, kernelSize, maxWidth)
}

class SlidingWindowSame[T <: Data](dataType: HardType[T], kernelSize: Int, maxWidth: Int) extends Component {
  val io = new Bundle {
  ...
  val line = LineBufferSame(dataType, maxWidth, kernelSize)
  val pixel = PixelBufferSame(Vec(dataType, kernelSize), kernelSize)
